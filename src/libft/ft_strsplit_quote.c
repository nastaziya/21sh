...
static char		**ft_intab(char *src, char **dest, int operator, char delimitor)
{
	...
	if ((dest[operator] = (char *)malloc(sizeof(*dest) * (i + 1))) == NULL)
		return (NULL);
	i = 0;
	while (isnotlegit(src[i], delimitor))
	{
...
char			**ft_strsplit_quote(char const *str, char delimitor,
					char replace)
{
	int		i;
	int		j;
	...
	if (!str)
		return (NULL);
	ft_replace_spaces_inside_quotes((char*)str, delimitor, replace);
